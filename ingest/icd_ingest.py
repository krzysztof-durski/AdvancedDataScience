    ...
    with open(path, encoding="utf-8", newline="") as f:
        ...
        for parts in iterable:
            ...
            try:
                ...
                counters.skipped += 1
                continue
            if row_type not in {1, 5}:
                counters.skipped += 1
                continue
            code = (parts[3] if row_type == 1 else parts[5]) if len(parts) > 5 else ""
...
def upsert_icd_rows(cur, rows: list[tuple], batch_size: int = 1000, retries: int = 3) -> tuple[int, int]:
    if not rows:
        return 0, 0
    ...
    upserted AS (
      INSERT INTO icd_reference(code, version_year, description_de, row_type, source_file, ingested_at)
