...
def parse_icd_txt(path: str | Path, version_year: int | None = None) -> Iterator[dict]:
    """
...
def ingest_icd_to_db(path: str | Path, conn, version_year: int | None = None) -> int:
    """
    Ingest ICD TXT into PostgreSQL. Returns count of inserted/updated rows.
    """
    records = list(parse_icd_txt(path, version_year))
    if not records:
        return 0
